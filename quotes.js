...
// Organized by emotional/situational context for intelligent matching

const quoteTree = {
  // Money, career, success struggles
  money: {
  ...
  // Haters, enemies, criticism
  haters: {
    keywords: ['hate', 'hater', 'enemy', 'jealous', 'critic', 'criticism', 'troll', 'attack', 'drama', 'beef', 'fight', 'against', 'cancel', 'cancelled', 'cancelled', 'gossip', 'rumors', 'talk', 'trash'],
    quotes: [
      "Love your haters - they're your biggest fans.",
...
/**
 * Find the best matching context for user input
 * Returns { context: string, score: number }
 */
function matchContext(userInput) {
  ...
  for (const [contextName, contextData] of Object.entries(quoteTree)) {
    ...
    for (const keyword of contextData.keywords) {
      ...
      // Partial match (keyword contained in input)
...
/**
 * Get a wisdom quote based on user's issue
 * Returns { quote: string, context: string }
 */
...
module.exports = { quoteTree, matchContext, getWisdom };
